...
def connect_milvus():
    ...
    try:
        client = MilvusClient(
            uri="http://" + MILVUS_CONFIG['host'] + ":" + str(MILVUS_CONFIG['port']),
            timeout=30
        )
        print("Milvus连接成功")
        return client
...
def create_collection(client, collection_name, dimension):
    ...
    try:
        ...
        if client.has_collection(collection_name):
            print("集合", collection_name, "已存在，将删除并重新创建")
...
def insert_chunks(client, collection_name, chunks, vectors):
    ...
    try:
        ...
        if entities:
            ...
            print("成功插入", insert_count, "条数据到", collection_name)
            return insert_count
        else:
            print("没有数据可插入")
            return 0
...
def process_pdf_and_create_collection(pdf_path, collection_name):
    ...
    try:
        ...
        print("读取完成，文本长度:", len(raw_text))
        
        # 2. 切分文本
        print("正在切分文本...")
        chunks = chunk_text(raw_text)
        print("切分完成，共", len(chunks), "个片段")
...
def main():
    ...
    if os.path.exists(psychology_pdf):
        ...
        print("心理PDF文件不存在:", psychology_pdf)
        ...
        if dialogue_pdfs:
            ...
            for pdf_path in dialogue_pdfs:
                ...
                try:
                    ...
                    print("读取完成，获取", len(chunks), "个对话片段")
                except Exception as e:
                    print("读取失败:", e)
                ...
                if vectors:
                    ...
                    if client:
                        ...
                        if create_collection(client, "dialogue_rag", vec_dim):
                            ...
                            print("对话模板处理完成，插入", insert_count, "条数据")
        else:
            print("没有找到对话模板PDF文件")
    else:
        print("对话模板目录不存在:", DIALOGUE_DIR)
