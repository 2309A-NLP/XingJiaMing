...
import { fetchChats, fetchMessages, createChatAPI, saveMessage, updateChatTitle, deleteChatAPI, queryStream, ingest, checkHealth } from './api'
...
export default function App() {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      const elapsed = startTime ? Math.round((Date.now() - startTime) / 100) / 10 : 0
      setChats(prev => prev.map(c => ({
        ...c,
