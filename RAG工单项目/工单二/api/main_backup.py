...
def _search_with_query_understanding(comp: dict, question: str, top_k: int, page_filter: list = None, language: str = "zh"):
    ...
    for query in search_queries:
          # 英文时，向量检索用原 query，BM25 用翻译后的中文 query
          search_for_bm25 = bm25_query if language == "en" else query
          results = retriever.search(query, top_k=top_k, page_filter=page_filter, bm25_query=search_for_bm25)
        for r in results:
