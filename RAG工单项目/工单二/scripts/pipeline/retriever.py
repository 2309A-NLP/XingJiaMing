...
class Retriever:
    ...
    def __init__(self, vector_store, bm25_retriever, embedder, reranker=None):
        ...
        self._reranker_path = None  # lazy load
    ...
    def search(self, query: str, top_k: int = 5,
        ...
        # 只取 top_k+3 个候选，减少 rerank 负担
        dense_results = self._vs.search(query_vec, top_k=top_k + 3)
        sparse_results = self._bm25.search(bm25_query or query, top_k=top_k + 3)
    ...
    def _load_reranker(self):
        """lazy load reranker, not at startup"""
        try:
    ...
    def _smart_rerank(self, merged: list, top_k: int, query: str) -> list:
        """智能 Rerank：用 hit_count 判断，节省 1-2 秒。

        规则：
        1. top-1 被双路命中且 top-k 只有单路命中 -> 跳过
        2. 候选数 <= top_k -> 跳过
        3. 其他情况 -> 只 rerank top 8 个候选
        """
        if len(merged) <= top_k:
            return merged
        ...
        # 只 rerank top 8 个候选
        reranked = self._reranker.rerank(query, merged[:8], top_k=top_k)
        return reranked
    ...
    def _merge(self, dense: List[dict], sparse: List[dict]) -> List[dict]:
        ...
        for rank, r in enumerate(dense, start=1):
            cid = r['chunk_id']
            seen[cid] = {**r, 'rrf_score': 1.0 / (RRF_K + rank), 'hit_count': 1}

        for rank, r in enumerate(sparse, start=1):
            cid = r['chunk_id']
            if cid in seen:
                seen[cid]['rrf_score'] += 1.0 / (RRF_K + rank)
