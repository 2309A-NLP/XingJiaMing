...
class ChatStore:
    ...
    def add_message(self, chat_id, msg_id, role, content, sources=None, timestamp=None, query_analysis=None, response_time=None):
        ...
        with self._conn() as conn:
            conn.execute('INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (msg_id, chat_id, role, content, sj, ts, qa, response_time))
    def get_chats(self):
        with self._conn() as conn:
    ...
    def get_messages(self, chat_id):
        with self._conn() as conn:
            rows = conn.execute('SELECT id, role, content, sources, timestamp, query_analysis, response_time FROM messages WHERE chat_id = ? ORDER BY timestamp', (chat_id,)).fetchall()
        result = []
        for r in rows:
