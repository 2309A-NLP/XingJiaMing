    ...
    def _process_batch(
        ...
        for pn in range(start, end + 1):
            ...
            if self._vision and self._vision.is_available:
                # 判断逻辑：OCR 文字少于 200 字 → 可能是图片页 → 调多模态
                # 纯文字页面 OCR 通常能提取大量文字，图片页面文字很少
                should_call_vision = len(md_text.strip()) < 200

                if should_call_vision:
                    try:
                        vision_desc = self._vision.analyze(image, pn)
                        if vision_desc:
                            vision_section = "\n\n**图片内容描述：**\n" + vision_desc
                    ...
                    logger.debug('第%d页: 纯文字页面（%d字），跳过多模态', pn, len(md_text.strip()))
