...
| 多轮对话 | Redis 短期记忆，支持上下文追问，记住用户名等个人信息 |
...
- Redis（localhost:6379，用于多轮对话短期记忆）
...
### 架构
- **短期记忆**：Redis 存储最近 10 轮对话（`chat:{chat_id}:history`），TTL 1 小时
- **持久化**：SQLite 存储完整对话记录（供历史查看）
- **上下文注入**：查询时从 Redis 读取历史，拼入 LLM messages，让模型理解上下文

### 流程
```
用户发消息（带 chat_id）
  ↓
从 Redis 读取该对话的最近 10 轮历史
  ↓
历史对话 + 检索结果 + 当前问题 → LLM
  ↓
LLM 生成回答（知道用户之前说了什么）
  ↓
本轮 Q&A 存回 Redis（供下次追问）
```

### 支持的场景
- 追问："那第二家呢？"（指代上一轮的公司）
- 个人信息记忆："我叫李达" → "我是谁？" → "你是李达"
- 上下文补充："它的注册资本呢？"（指代上一轮提到的公司）
...
MILVUS_PORT=19530
MILVUS_COLLECTION=rag_workorder5
