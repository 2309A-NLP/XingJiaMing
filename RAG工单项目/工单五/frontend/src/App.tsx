...
import type { Chat, Message, QueryAnalysis, QueueStatus } from './api'
import { fetchChats, fetchMessages, createChatAPI, saveMessage, updateChatTitle, deleteChatAPI, queryStream, ingestMultiple, checkHealth, selfCheck, subscribeProgress } from './api'
...
export default function App() {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      await queryStream(text, 5, hasChinese ? 'zh' : 'en', cid,
  ...
  const handleUpload = async (files: File[]) => {
    ...
    try {
      const data = await ingestMultiple(files)
    } catch (e: any) {
      addMessageToState(cid, { 
