...
class BGEReranker(BaseReranker):
    ...
    def __init__(self, model_path: str, device: str = 'cpu'):
        ...
        if device == 'cuda' and torch.cuda.is_available():
            actual_device = 'cuda'
            self._model = self._model.to(actual_device).eval()
    ...
    def rerank(self, query: str, candidates: List[dict], top_k: int = 5) -> List[dict]:
        ...
        with torch.no_grad():
            logits = self._model(**inputs).logits.view(-1).detach().cpu().tolist()

        sorted_pairs = sorted(zip(logits, candidates), key=lambda x: x[0], reverse=True)
