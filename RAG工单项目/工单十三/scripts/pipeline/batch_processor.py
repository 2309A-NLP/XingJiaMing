...
from scripts.pipeline.text_cleaner import PageCleaner
...
from api.progress import start as progress_start, update_batch as progress_batch, update_stage as progress_stage, finish as progress_finish
...
class BatchProcessor:
    ...
    def __init__(
        ...
        if vision_enabled:
            ...
            if not self._vision.is_available:
                logger.warning('VISION_ENABLED=true 但 API 未配置，图片解析将跳过')
                self._vision = None
        else:
            self._vision = None
    ...
    def process(self) -> IngestReport:
        ...
        # 初始化进度追踪
        progress_start(pdf_name, total_pages, total_batches)
        ...
        for batch_idx in range(total_batches):
            ...
            progress_batch(batch_no, end_p - start_p + 1, batch_elapsed)
            self._force_gc()
    ...
    def _render_all_pages(self, total_pages: int) -> Dict[int, Image.Image]:
        ...
        try:
            ...
            for i in range(total_pages):
                ...
                if (i + 1) % 50 == 0:
                    gc.collect()
                    logger.debug("已渲染 %d/%d 页", i + 1, total_pages)
        except Exception as e:
            logger.error("渲染页面失败（第%d页）: %s", len(images) + 1, e)
            raise
        finally:
            if pdf:
                pdf.close()

        logger.info("页面图片: %d/%d", len(images), total_pages)
        return images
    ...
    def _process_batch(
        ...
        for pn in range(start, end + 1):
            ...
            if self._vision and self._vision.is_available:
                try:
                    # 判断逻辑：
                    # 1. OCR 文字少于 200 字 → 可能是图片页
                    # 2. 文字包含图表关键词（如"市场应用结构与增长"）→ 可能是图表页
                    text_len = len(md_text.strip())
                    short_text_threshold = int(os.getenv('VISION_SHORT_TEXT_THRESHOLD', '200'))
                    has_chart_text = self._vision.should_analyze_text(md_text)

                    should_call_vision = text_len < short_text_threshold or has_chart_text

                    if should_call_vision:
                        # 图表页整页可能被正文/水印干扰，尝试裁剪图表区域
                        vision_desc = self._vision.analyze(image, pn, force=True)
                        if vision_desc:
                            vision_section = "\n\n**图片内容描述：**\n" + vision_desc
                            full_md = full_md + vision_section
                            engines_used.append('vision')
                            logger.info('第%d页: 图片语义解析完成', pn)
                    else:
                        logger.debug('第%d页: 纯文字页面（%d字），跳过多模态', pn, text_len)
                except Exception as e:
                    logger.warning('第%d页: 图片语义解析失败: %s', pn, str(e)[:100])

            # 保存解析结果（必须放在多模态拼接之后，否则图片描述不会进入 refined.md）
