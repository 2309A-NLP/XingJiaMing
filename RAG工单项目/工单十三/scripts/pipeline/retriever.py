...
class Retriever:
    ...
    def __init__(self, vector_store, bm25_retriever, embedder, reranker=None):
        ...
        self._rerank_candidate_k = int(os.getenv('RERANK_CANDIDATE_K', '12'))
        self._rerank_candidate_k_cpu = int(
            os.getenv('RERANK_CANDIDATE_K_CPU', str(min(self._rerank_candidate_k, 6)))
        )
    ...
    def _smart_rerank(self, merged: list, top_k: int, query: str) -> list:
        ...
        # 只 rerank 固定数量候选，避免重排成为主瓶颈
        candidate_k = self._rerank_candidate_k
        if getattr(self._reranker, '_device', None) == 'cpu':
            candidate_k = min(candidate_k, self._rerank_candidate_k_cpu)
        reranked = self._reranker.rerank(query, merged[:candidate_k], top_k=top_k)
        return reranked
