...
def _warm_up_components(components: Dict[str, Any]) -> None:
    """预热关键组件，降低首个请求延迟。"""
    try:
        ...
        if retriever and getattr(retriever, '_reranker', None):
            sample = [
                {'chunk_id': 'warmup-1', 'content': 'warmup content 1', 'hit_count': 1},
                {'chunk_id': 'warmup-2', 'content': 'warmup content 2', 'hit_count': 1},
            ]
            retriever._smart_rerank(sample, 1, 'warmup')
        if generator:
            generator._build_messages('warmup', 'warmup')
        logger.info('关键组件预热完成')
    except Exception as e:
        logger.warning('关键组件预热失败: %s', e)
...
def get_components() -> Dict[str, Any]:
    ...
    with _components_lock:
        ...
        collection = os.getenv('MILVUS_COLLECTION', 'rag_workorder6')
        ...
        if os.getenv('RERANK_ENABLED', 'false').lower() == 'true':
            ...
            if rerank_path:
                ...
                reranker = create_reranker(reranker_type='bge', model_path=rerank_path, device='cuda')
            else:
        ...
        _warm_up_components(_components)
