...
import type { Chat, Message, QueueStatus, SearchConfig } from './api'
import { DEFAULT_SEARCH_CONFIG, fetchChats, fetchMessages, createChatAPI, saveMessage, updateChatTitle, deleteChatAPI, queryStream, queryMergeStream, ingestMultiple, checkHealth, selfCheck, subscribeProgress } from './api'
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      await queryStream(text, searchConfig.top_k, hasChinese ? 'zh' : 'en', cid,
        ...
        // onLightRAGResult: 收到LightRAG结果后触发融合
        async (lightragResult) => {
          lightragAnswer = lightragResult.answer || ''
          ragAnswer = aiContent
          // 立即存储LightRAG结果到消息（对比按钮可以直接用）
          if (lightragAnswer) {
            setChats(prev => prev.map(c => ({ ...c, messages: c.messages.map(m => m.id === aiId ? { ...m, rag_answer: ragAnswer, lightrag_answer: lightragAnswer } : m) })))
          }
          if (!lightragAnswer || lightragResult.error) return // LightRAG失败则不融合
          // 开始融合：先清空内容，显示融合中状态
          aiContent = ''
          setChats(prev => prev.map(c => ({ ...c, messages: c.messages.map(m => m.id === aiId ? { ...m, content: '🔄 正在融合回答...' } : m) })))
          // 流式融合
          await queryMergeStream(text, ragAnswer, lightragAnswer,
            (token) => { aiContent += token; setChats(prev => prev.map(c => ({ ...c, messages: c.messages.map(m => m.id === aiId ? { ...m, content: aiContent } : m) }))) },
            () => {}
          )
          // 融合完成，保存最终结果
          const elapsed = Math.round((Date.now() - startTime) / 100) / 10
          setChats(prev => prev.map(c => ({ ...c, messages: c.messages.map(m => m.id === aiId ? { ...m, sources: aiSources, query_analysis: aiQueryAnalysis, responseTime: elapsed, search_config: aiSearchConfig, rag_answer: ragAnswer, lightrag_answer: lightragAnswer } : m) })))
          saveMessage(cid, { id: aiId, role: 'assistant', content: aiContent, sources: aiSources, query_analysis: aiQueryAnalysis, responseTime: elapsed, search_config: aiSearchConfig, rag_answer: ragAnswer, lightrag_answer: lightragAnswer, timestamp: Date.now() }).catch(() => {})
        },
        // onDone: 传统RAG完成
        () => { setLoading(false) }
      )
      // 如果没有LightRAG结果（超时或失败），直接保存传统RAG结果
      if (!lightragAnswer) {
        const elapsed = Math.round((Date.now() - startTime) / 100) / 10
        setChats(prev => prev.map(c => ({ ...c, messages: c.messages.map(m => m.id === aiId ? { ...m, sources: aiSources, query_analysis: aiQueryAnalysis, responseTime: elapsed, search_config: aiSearchConfig, rag_answer: aiContent } : m) })))
        saveMessage(cid, { id: aiId, role: 'assistant', content: aiContent, sources: aiSources, query_analysis: aiQueryAnalysis, responseTime: elapsed, search_config: aiSearchConfig, rag_answer: aiContent, timestamp: Date.now() }).catch(() => {})
      }
    } catch (e: any) {
