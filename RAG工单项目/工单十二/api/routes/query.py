...
async def query_compare(req: QueryRequest):
    ...
    results = _search(comp, req)
    contexts = _build_context_from_results(results)
    # 并行调用 RAG 和纯 LLM，减少等待时间
    with ThreadPoolExecutor(max_workers=2) as executor:
        future_rag = executor.submit(comp['generator'].generate, req.question, contexts, req.language)
        future_llm = executor.submit(comp['generator'].generate, req.question, [], req.language)
        rag_answer = future_rag.result()
        llm_answer = future_llm.result()
...
async def query_stream(req: QueryRequest):
    ...
    async def generate():
        ...
        # 后台触发 LightRAG 查询（不阻塞主响应）
        try:
            from scripts.pipeline.lightrag_service import aquery_lightrag
            lightrag_answer = await aquery_lightrag(question=req.question, mode="hybrid")
            yield f"data: {json.dumps({'type': 'lightrag_result', 'data': {'answer': lightrag_answer}}, ensure_ascii=False)}\n\n"
        except Exception as e:
            logger.warning('LightRAG 后台查询失败: %s', e)
            yield f"data: {json.dumps({'type': 'lightrag_result', 'data': {'answer': '', 'error': str(e)}}, ensure_ascii=False)}\n\n"
