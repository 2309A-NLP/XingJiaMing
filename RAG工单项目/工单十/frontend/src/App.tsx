...
export default function App() {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      setChats(prev => prev.map(c => ({ ...c, messages: c.messages.map(m => m.id === aiId ? { ...m, sources: aiSources, query_analysis: aiQueryAnalysis, responseTime: elapsed, search_config: aiSearchConfig } : m) })))
      saveMessage(cid, { id: aiId, role: 'assistant', content: aiContent, sources: aiSources, query_analysis: aiQueryAnalysis, responseTime: elapsed, search_config: aiSearchConfig, timestamp: Date.now() }).catch(() => {})
    } catch (e: any) {
