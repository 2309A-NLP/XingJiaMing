...
- temporal: 时间性问题（询问时间、日期、时期）
- quantitative: 数量性问题（询问数字、金额、比例）
- management: 管理层问题（询问董事长、行长、高管等）
- other: 其他类型

【重要规则】
1. 如果问题是关于管理层（董事长、行长、CEO等），intent 必须设为 "management"
2. 对于管理层问题，sub_queries 应该包含：
   - 原始问题
   - 拆分后的子问题，如"XXX公司的董事长是谁？"和"XXX公司的行长是谁？"
3. keywords 应该包含：公司名称、年份、职位名称（董事长、行长、CEO等）
4. 如果问题本身已经很清晰，disambiguated_query 可以和原问题一样
5. 如果问题很简单，sub_queries 只需要一个元素（原问题）
6. confidence 表示你对分析结果的信心（0-1）"""
